...
За бажанням клієнт запускається разом із Windows у фоновому режимі. Коли вікно видиме, перевірки API виконуються частіше; у фоновому режимі — навмисно рідше, щоб заощаджувати ресурси. Прихований клієнт також може виявити важливу локальну зміну файлу локалізації. Сам клієнт додатково перевіряє наявність новішої версії приблизно кожні 5 хвилин; якщо нову версію знайдено у прихованому режимі, Windows показує одне сповіщення для цієї версії. Після відкриття програми доступна звичайна кнопка оновлення. Пункт **"Перевірити зараз"** перевіряє і локалізаційний фід, і доступність нової версії клієнта. Сповіщення є інформаційним і не запускає оновлення автоматично; для відкриття використовуйте трей.

Фоновий режим розрахований на роботу без помітного навантаження на комп’ютер. На наведеному вимірюванні в Диспетчері завдань BDO UA Client споживає **0% CPU**, близько **17,6 МБ оперативної пам’яті**, а активність диска та мережі становить **0 Мбіт/с**. Фактичні показники можуть трохи відрізнятися залежно від системи та моменту перевірки, але клієнт виконує фоновий моніторинг ощадливо й не заважає повсякденній роботі.

![BDO UA Client у фоновому режимі — споживання ресурсів](docs/img/background-resource-usage.png)
...
BDO-UA Client — portable-застосунок: він не встановлюється у Windows через MSI/MSIX або Windows Installer, тому окремого деінсталятора у **Programs and Features** немає.

### Звичайне видалення

1. Якщо увімкнено автозапуск, спочатку вимкніть його в меню іконки трея **«Запускати разом із Windows»**.
2. Повністю завершіть клієнт через меню трея **«Вихід»**. Натискання **X** лише ховає програму у трей і не завершує процес.
3. Видаліть файл **`BDO-UA-Client.exe`**.

### Повне очищення даних

Окремо від EXE можуть залишатися дані клієнта у `%LocalAppData%\BDO-UA-Client`: конфігурація, логи, стан встановлення, тимчасові файли cache, резервні копії та дані сесій оновлення. Якщо ці дані більше не потрібні, після повного завершення клієнта можна видалити всю цю папку. Логи та резервні копії можна залишити, якщо вони потрібні для діагностики або відновлення.

Видалення клієнта або його папки даних **не відновлює і не видаляє локалізацію, вже застосовану до Black Desert Online**. Якщо потрібно повернути оригінальну локалізацію гри, перед видаленням клієнта використайте його дію **«Відновити оригінал»**. Якщо локалізацію потрібно залишити, цю дію виконувати не потрібно.
