...
├── Program.cs                (composition root, startup sequence)
├── MainForm.cs               (UI logic, event handlers, state refresh)
├── MainForm.Designer.cs      (WinForms layout: 4 блоки)
│
├── Api/
│   ├── BdoUaApiClient.cs     (HTTP client, /releases endpoint)
│   └── ApiResult.cs          (generic Result<T> з ApiErrorKind)
│
├── Models/
│   ├── ReleasesResponse.cs   (Success, GeneratedAt, Data)
│   ├── ReleaseData.cs        (OfficialPatch, OfficialSourceUrl, Modes, Progress...)
│   ├── LocalizationMode.cs   (Slug, PublicName, Current, History)
│   ├── CurrentRelease.cs     (PublicId, Version, DownloadUrl, Sha256, CompatibleWithOfficialPatch...)
│   ├── ReleaseHistoryItem.cs (PublicId, Version, Patch, Status, PublishedAt, RetiredAt)
│   ├── InstallPathPattern.cs (Pattern, Launcher, Description)
│   ├── GameTestInfo.cs       (State, Label, Note)
│   ├── ProgressInfo.cs       (TotalRows, TranslatedPercent, ManualRows, MachineRows...)
│   ├── StatsInfo.cs          (RowsInFile)
│   ├── AnnouncementsInfo.cs  (DiscordReleases, TelegramMain)
│   ├── BackupMetadata.cs     (CreatedAt, GamePatch, Sha256, SizeBytes, Source, InstallationState)
│   ├── RestorePointInfo.cs   (Id, CreatedAt, GamePatch, Source, IsRestorable...)
│   └── RestoreResult.cs      (RestoreError enum + RestoreResult)
│
├── Services/
│   ├── GameDetector.cs                   (saved→registry→Steam→API→manual detection)
│   ├── DetectionResult.cs                (DetectionSource enum + DetectionResult)
│   ├── LocalizationInstaller.cs          (download + SHA-256 + retry + progress)
│   ├── DownloadResult.cs                 (DownloadError enum + DownloadResult)
│   ├── LocalizationInstallService.cs     (transactional install + rollback)
│   ├── InstallResult.cs                  (InstallError enum + InstallResult)
│   ├── LocalizationStateService.cs       (state resolution: NotInstalled/UpToDate/...)
│   ├── LocalizationState.cs              (LocalizationState enum)
│   ├── LocalizationStateResult.cs        (State + diagnostic Error)
│   ├── LocalizationCompatibilityService.cs (compatibility check)
│   ├── CompatibilityResult.cs            (IsAllowed + Reason)
│   ├── RestoreOriginalService.cs         (download official → fallback snapshot)
│   ├── RestoreBackupService.cs           (restore from restore point + rollback)
│   ├── HashHelper.cs                     (SHA-256 compute + file copy)
│   ├── DynamicModePolicy.cs              (mode filtering, validation, display formatting)
│   ├── InstallActionPolicy.cs            (button availability logic)
│   └── OperationState.cs                 (OperationState enum: Idle/Downloading/...)
│
├── Storage/
│   ├── AppPaths.cs            (%LocalAppData%\BDO-UA-Client\ structure)
│   ├── Config.cs              (GamePath, LastMode)
│   ├── ConfigStore.cs         (load/save config.json)
│   ├── FileLoadResult.cs      (FileLoadStatus enum: Missing/Valid/Invalid)
│   ├── InstallationMetadata.cs (ModeSlug, PublicId, Version, GamePatch, Sha256, Source)
│   ├── InstallationStateStore.cs (load/save/validate installation.json)
│   └── BackupStore.cs         (original snapshot + restore points + replace + recovery)
│
├── Logging/
│   ├── ILogger.cs             (interface: Debug/Info/Warning/Error)
│   └── FileLogger.cs          (daily log files, thread-safe, non-throwing)
│
└── BdoClient.Tests/          (net8.0-windows, xUnit)
    ...
    ├── Models/
    │   └── ReleasesResponseTests.cs
    ├── Logging/
    │   └── FileLoggerTests.cs
    ├── Services/
    │   ├── GameDetectorTests.cs
    │   ├── LocalizationInstallerTests.cs
    │   ├── LocalizationInstallServiceTests.cs
    │   ├── LocalizationStateServiceTests.cs
    │   ├── LocalizationCompatibilityServiceTests.cs
    │   ├── RestoreOriginalServiceTests.cs
    │   ├── RestoreBackupServiceTests.cs
    │   ├── InstallActionPolicyTests.cs
    │   └── DynamicModePolicyTests.cs
    └── Storage/
        ├── AppPathsTests.cs
        ├── ConfigStoreTests.cs
        ├── InstallationStateStoreTests.cs
        └── BackupStoreTests.cs
```
...
- Mode Selection блок: `modesFlowPanel` для динамічних RadioButtons (API-driven)
- Status блок: state label, installed info label, details label, ProgressBar, progress label, multiline message TextBox
- Actions блок: 3 buttons (Install / Restore Original / Cancel), disabled by default
- DPI-aware layout via AutoScaleMode.Font + TableLayoutPanel + AutoSize
- Resize-safe: MinimumSize 620×480, ClientSize 640×560, percent-based columns, AutoEllipsis для game path
- Presentation helper methods: SetGamePathText, SetLocalizationStateText, SetInstalledInfo, SetDetailsText, SetProgress, SetMessage, SetActionsEnabled, GetSelectedModeSlug
- UI без business logic: без HttpClient, без API calls, без file operations, без config/state

**Примітка (v12.1):**
- Restore Backup видалено з UI (backend retained)
- Update button видалено — єдина кнопка "Встановити" для всіх сценаріїв
- RadioButtons будуються динамічно з API `_apiResponse.Data.Modes`

**Acceptance criteria:**
- [x] Одне вікно, вертикальний layout
- [x] Блок Game Detection (пошук + ручний вибір)
- [x] Блок Mode Selection (динамічні radio buttons з API)
- [x] Блок Status (state + progress bar)
- [x] Блок Actions (Install / Restore Original / Cancel)
...
- No hardcoded mode slugs in production UI
...
- [x] OperationState оновлює UI (Stage 10)
