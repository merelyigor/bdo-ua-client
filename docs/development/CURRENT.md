...
Current phase: Stage C — MainForm physical decomposition — COMPLETED / REVIEWED / ACCEPTED (C.1–C.5 physical implementation complete; final architect review / lifecycle handoff completed)
...
1. Stage C COMPLETED / REVIEWED / ACCEPTED (C.1–C.5 фізична декомпозиція + фінальний architect review/lifecycle handoff). Наступна дія вимагає явного owner-рішення: активувати BACKLOG `background-tray-notifications` або обрати інше завдання.
2. Do not automatically start Stage B before the tray/background feature and explicit owner activation.
