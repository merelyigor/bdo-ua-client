...
1. Stage C COMPLETED / REVIEWED / ACCEPTED (C.1–C.5 фізична декомпозиція + фінальний architect review/lifecycle handoff). Owner явно активував `background-tray-notifications` (ACTIVE); **T1 — Tray lifetime shell — COMPLETED / REVIEWED / OWNER ACCEPTED**; **T1.1 — Autostart / background startup — COMPLETED / REVIEWED / ACCEPTED** (single-instance безпека включно); **T2 — Operation / shutdown semantics — COMPLETED / REVIEWED / ACCEPTED**; **T3 — Background polling cadence — COMPLETED / REVIEWED / ACCEPTED**; наступна tray-задача — **T5 — Notifications / dedup** (T4 — Local file-change trigger — COMPLETED / REVIEWED / ACCEPTED).
2. Do not start Stage B before tray/background feature is implemented and reviewed.
