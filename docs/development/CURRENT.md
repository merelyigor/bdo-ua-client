...
Current phase: Stage C — MainForm physical decomposition (C.1–C.5 physical implementation complete; pending final architect review / explicit lifecycle handoff)
...
1. Stage C final review / explicit lifecycle handoff — після нього окремий BACKLOG `background-tray-notifications`. C.5 ApplicationUpdate extraction уже COMPLETED (10 self-update методів перенесено у `MainForm.ApplicationUpdate.cs`, без зміни поведінки).
