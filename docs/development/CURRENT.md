...
1. Stage C COMPLETED / REVIEWED / ACCEPTED (C.1–C.5 фізична декомпозиція + фінальний architect review/lifecycle handoff). Owner явно активував `background-tray-notifications` (ACTIVE); **T1 — Tray lifetime shell — COMPLETED / REVIEWED / OWNER ACCEPTED**; наступна технічна дія — **T1.1 — Autostart / background startup** (НЕ реалізовано).
