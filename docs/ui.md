...
- **Persistent status**: після успішного пошуку `gameStatusLabel` залишається зеленим. Наступний невдалий пошук не перезаписує попередній валідний стан — якщо новий пошук помилково не знайшов гру, а попередній шлях досі валідний, статус зберігається.
- **Manual parent→child resolution**: якщо користувач обирає батьківську папку (наприклад, `C:\Games`), `GameDetector.ResolveManualGameRoot` шукає підпапку з грою. Якщо знайдено кілька — повідомлення "Знайдено кілька папок з грою. Оберіть точну папку гри."
- **Preserves valid root on invalid pick**: якщо користувач обрав невалідну папку, а попередній `_gameRoot` досі валідний (`ValidateGamePath` повертає `true`), статус гри не змінюється — показується лише тимчасове повідомлення про помилку.
